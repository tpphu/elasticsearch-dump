...
class elasticsearch {
  constructor (parent, url, options) {
    ...
    this.totalOrRemainSearchResults = -1
    ...
    /**
     * Note that searchByScroll vs SearchByOffset
     */
    this.isSearchByScroll = true
    if (this.parent.options.offsetFieldName)  {
  ...
  get (searchKeeperInfo, callback) {
    const type = this.parent.options.type
    this.version('input', err => {
      if (err) { return callback(err) }
      let limit = searchKeeperInfo.limit
      let offset = searchKeeperInfo.offset
      if (type === 'data') {
  ...
  version (prefix, callback) {
    ...
    this.baseRequest(esRequest, (err, response) => {
      ...
      if (!this.isSearchByScroll) {
        if (this.searchBody.query.bool) {
          this.searchBody.query.bool.filter = [{
            "range": {
              [this.parent.options.offsetFieldName]: {
                "gt": this.parent.options.offset
              }
            }
          }]
          this.searchBody.sort = {
            [this.parent.options.offsetFieldName]: 'asc'
          }
        }
      }
  ...
  getData (limit, offset, callback) {
    ...
    if (this.isSearchByScroll) {
      if( offset >= this.totalOrRemainSearchResults && this.totalOrRemainSearchResults >= 0 ){
        callback(null, [])
        return
      }
    } else if ( this.totalOrRemainSearchResults === 0 ){
      callback(null, [])
      return
    }
    

    if (this.isSearchByScroll && this.lastScrollId !== null) {
      console.log('lastScrollId:', this.lastScrollId)
      ...
      if (this.isSearchByScroll && !this.elementsToSkip) { this.elementsToSkip = offset }
      ...
      if( this.isSearchByScroll ) {
        ...
        uri = `${this.base.url}/_search?${additionalParams}`
      }

      searchBody.size = this.parent.options.size >= 0 && this.parent.options.size < limit ? this.parent.options.size : limit

      if (this.parent.options.offsetFieldName) {
        _.set(searchBody.query, 'bool.filter[0].range.' + this.parent.options.offsetFieldName + '.gt', offset)
      }
      ...
      this.baseRequest(searchRequest, (err, response) => {
        ...
        if (this.isSearchByScroll && this.lastScrollId === undefined) {
          err = new Error('Unable to obtain scrollId; This tends to indicate an error with your index(es)')
          callback(err, [])
          return
        }
        this.totalOrRemainSearchResults = this.parent.options.size >= 0 ? this.parent.options.size : body.hits.total
        console.log('Remain result: ', this.totalOrRemainSearchResults)
  ...
  // return (error, writes)
  set (data, searchKeeperInfo, callback) {
    const type = this.parent.options.type
      ...
      this.baseRequest(esRequest, (err, response) => { // ensure the index exists
        ...
        for (let index in data) {
          ...
          async.eachSeries(sortedMappings, (set, done) => {
            const url = `${this.base.url}/${encodeURIComponent(set.key)}/_mapping`
            const payload = {
              url: url,
              method: 'PUT',
  ...
  setAnalyzer (data, limit, offset, callback) {
    const updateAnalyzer = (err, response) => {
      ...
      for (let index in data) {
        ...
        for (let key in settings) { // iterate through settings
          const setting = {}
          ...
          for (let p in setting[key]) { // iterate through index
